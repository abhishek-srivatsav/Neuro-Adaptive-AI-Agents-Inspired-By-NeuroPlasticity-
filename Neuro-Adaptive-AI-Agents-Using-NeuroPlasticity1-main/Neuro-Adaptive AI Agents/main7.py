...
def run_multi_agent_query(agent_type="normal", image_path=None, audio_path=None, text_input=None, top_n=5):
    """Run query with specific agent type and safely extract facts/knowledge."""

    print(f"\n🤖 {agent_type.upper()} AGENT ACTIVATED")
    print("=" * 50)
    ...
    #print("✅ Agent analysis complete:", results)

    # ---------------- Safe tuple/dict handling ----------------
    if agent_type == "mute":
        # Unwrap tuple if returned
        if isinstance(results, tuple):
            results = results[0] if results and isinstance(results[0], dict) else {}
        elif not isinstance(results, dict):
            results = {}

        # Handle nested tuple or dict for knowledge_result
    ...
    print(f"AGENT TYPE: {agent_type_res}")
    print(f"REASONED SENTENCE: {reasoned_sentence}")
    ...
    if knowledge_result:
        ...
        for fact_item in facts:
            if isinstance(fact_item, dict):
                ...
                facts_list.append({"fact": str(fact_item)})
        ...
        for knowledge_item in knowledge:
            if isinstance(knowledge_item, dict):
                ...
                knowledge_list.append({"knowledge": str(knowledge_item)})

        print("KNOWLEDGE OUTPUT:")
        print(json.dumps(knowledge_result, indent=4))

    print(f"\n✅ Extracted {len(facts_list)} facts and {len(knowledge_list)} knowledge items")
    ...
    if agent_type == "normal":
        ...
        if isinstance(explanation, dict):
            ...
            if "audio" in explanation:
                print(f"🔊 Audio file saved: {explanation['audio']}")

    elif agent_type == "blind":
        # Generate blind agent audio explanation
        explanation = gemini_flash.generate_blind_audio_explanation(
            query=query_text,
            facts=facts_list,
            knowledge=knowledge_list
        )
        print("\n🎧 BLIND AUDIO EXPLANATION:")
        if isinstance(explanation, dict) and "audio_file" in explanation:
            print(f"🔊 Blind audio explanation saved: {explanation['audio_file']}")
        else:
            print("⚠️ Audio file was not generated.")

        # ---------------- Generate feedback for Mute agent from Blind agent ----------------
        blind_audio_text = explanation.get("audio_text", "")
        #print("Blind Audio Text:", blind_audio_text)
        ...
        print("Mute Explaination",mute_feedback)
        print("\n✍️ MUTE FEEDBACK GENERATED FROM BLIND AGENT:")
        if isinstance(mute_feedback, dict) and "text" in mute_feedback:
            print(f"📝 Mute agent text feedback:\n{mute_feedback['text']}")
        else:
            print("⚠️ Mute feedback text was not generated.")
        ...
        print("\n✍️ MUTE TEXT EXPLANATION:")
        if isinstance(explanation, dict) and "text" in explanation:
            print(f"📝 Mute explanation text: {explanation['text']}")
            blind_feedback = gemini_flash.generate_agent_feedback(
                input_text=explanation['text'],
                agent_from="mute",
                agent_to="blind",
                context={"facts": facts_list, "knowledge": knowledge_list}
            )
            # Print results
            print("\n🎯 BLIND FEEDBACK GENERATED FROM MUTE AGENT:")
            if isinstance(blind_feedback, dict) and "audio_file" in blind_feedback:
                print(f"🔊 Blind agent audio feedback saved: {blind_feedback['audio_file']}")
            else:
                print("⚠️ Blind feedback audio was not generated.")
        else:
            print("⚠️ No mute explanation generated")
        

    # ---------------- Attach to results ----------------
    results["human_like_explanation"] = explanation
    results["facts_list"] = facts_list
    results["knowledge_list"] = knowledge_list
...
if __name__ == "__main__":
    ...
    # Demo all agents
    agents_to_test = ["normal","blind","mute"]  
